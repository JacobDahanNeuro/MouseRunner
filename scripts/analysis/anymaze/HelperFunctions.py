...
def file_sort(Files,data_constants,overwrite_id,key):
    ...
    if data_constants:
        MatchedFiles, data_constants = match_constants(Files,MatchedFiles,data_constants,overwrite_id,key)
    return MatchedFiles, data_constants    

def stat_compare(df,dtype='normalized_freezing',by='cue',alpha=0.05):
    anova       = stats.f_oneway(*[getattr(df.groupby([by]).get_group(x),dtype) for x in\
                                   getattr(df,by).unique()])
...
def search_tuples(iterables,key_to_find):
    return tuple(filter(lambda x:key_to_find in x, iterables)) 
    
def generate_pairs(df,x,hue):
    pairs = list(product(df[x].unique(),df[hue].unique()))
    matched_pairs = [search_tuples(pairs,key) for key in df[x].unique()]
